...
let complete (collection : IMongoCollection<Todo>) (id : string) : Option<Todo> =
  let todo = collection.Find(fun x -> x.Id = id).ToList()
              |> Seq.tryHead

  if Option.isSome todo then
    let filter = Builders<Todo>.Filter.Eq((fun x -> x.Id), id)
    let update =
      Builders<Todo>.Update
        .Set((fun x -> x.Done), not (todo.Value.Done))
...
type IServiceCollection with
  member this.AddTodoMongoDB(collection : IMongoCollection<Todo>) =
    ...
    this.AddSingleton<TodoComplete>(complete collection) |> ignore
